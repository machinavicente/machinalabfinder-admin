  ...
  <div v-if="visible" class="modal-backdrop" @click.self="$emit('close')">
    <div class="modal-window">
      <h5 class="modal-title">
        <i class="bi me-2" :class="editMode ? 'bi-pencil text-warning' : 'bi-plus-circle text-primary'"></i>
      ...
      <form @submit.prevent="$emit('submit', formData)">
        ...
        <div class="mb-3">
          ...
          <input v-model="formData.enlace" type="url" class="form-control" required />
        </div>
        <div class="row">
          <div class="col-md-6 mb-3">
            <label class="form-label">Categoría</label>
            <input v-model="formData.categoria" type="text" class="form-control" list="categoriasList" required />
            <datalist id="categoriasList">
              <option v-for="cat in categorias" :key="cat" :value="cat"></option>
            </datalist>
          </div>
          <div class="col-md-6 mb-3">
            ...
            <select v-model="formData.asignatura" class="form-select" required>
              ...
              <option v-for="asig in asignaturas" :key="asig" :value="asig">{{ asig }}</option>
            </select>
          </div>
        </div>
        <div class="mb-3">
        ...
        <div class="d-flex justify-content-end gap-2">
          ...
          <button type="submit" class="btn" :class="editMode ? 'btn-warning' : 'btn-primary'">
